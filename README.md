...
- **Two stage.** Liberal BH screening at $\alpha = 0.20$ followed by
  ...
  $\rho$. Recommended when $n$ is large relative to $p$, in which case
  pure FDR control tends to retain statistically significant edges
  with small HSIC effect size.
...
│   └── 08_wine_robustness.R       # Robustness study
├── cluster/                       # SLURM batch submission scripts
├── data/                          # Bundled and auto-downloaded data
...
The `cluster/` directory contains SLURM batch submission scripts for
running the simulation grid on a high-performance computing cluster.
These scripts are not required for any of the analyses in the paper;
each script under `scripts/` runs end-to-end from the command line
through a single `Rscript` invocation, with no cluster dependency.
...
`scripts/02_simulation_linear.R` and does not pass through
`scripts/05_plot_simulations.R`. Wine data are downloaded from the
UCI Machine Learning Repository on first run and cached in `data/`.
...
| Spline basis      | cubic B      | cubic B           | `s(..., bs = "cr")` with REML smoothing           |
